...
U = [zeros(1,1*Fs),1,zeros(1,length(t)-1*Fs-1)];
U_eff = 0;
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
for i = 1:length(t)
    U_eff_dot = (U(i) - U_eff)/0.03;
    U_eff = U_eff_dot/Fs + U_eff;
    ...
    for n_sag = index_slow+1:N_MU
        S_af_temp(n_sag) = sag_function(S_af_temp(n_sag),f_env(n_sag),Fs);
        S_af(n_sag,i) = S_af_temp(n_sag);
    end
    
    for n_f = 1:N_MU        
        if n_f <= index_slow
            Af(n_f) = Af_slow_function(f_env(n_f),Lce,Y_af(n_f,i));
            OutputAf(n_f,i)= Af(n_f);
        else
            Af(n_f) = Af_fast_function(f_env(n_f),Lce,S_af(n_f,i));
            OutputAf(n_f,i)= Af(n_f);
        end
    end
    for n = 1:length(find(FR_MU(:,i)>=MFR_MU)) % loop through motor units whose firing rate is greater than minimum firing rate defined by the user
        ...
        if ~any(spike_train(n,:)) % when the motor unit fires at the first time
            ...
            if spike_time(n) == i % when the motor unit fires
                ...
                twitch = twitch_function(f_env(n),Af(n),Lce,CT(n),RT(n),Fs);
                twitch = Pi(n).*twitch;
                twitch_temp =  Pi(n).*t_twitch./Tf(n).*exp(1-t_twitch./Tf(n));
                force_temp = conv(spike_train_temp,twitch_temp);
                force(n,:) = force(n,:) + force_temp(1:length(t));
...
tesetdUnit = 1;
figure(1)
plot(t,Outputfenv(tesetdUnit,:))
hold on 
plot(t,Outputfint(tesetdUnit,:))
hold on 
plot(t,Outputfeff(tesetdUnit,:))
legend('f_{env}','f_{int}','f_{eff}')
hold off

figure(2)
plot(t,force(tesetdUnit,:)*1000)
xlabel('Time (s)')
ylabel('Force (mN)')
...
function twitch = twitch_function(f_env,Af,Lce,CT,RT,Fs)
T1 = (CT*2/3)*Lce^2+(CT*1/3)*f_env;              
T2_temp = ((RT*2/3) + (RT*1/3)*Af)/Lce;
T2 = T2_temp/1.68;
t_twitch = 0:1/Fs:1;

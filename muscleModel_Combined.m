...
for i = 1:length(t)
    ...
    for f_temp = 1:N_MU
        [f_int(f_temp),f_int_dot(f_temp),Tf(f_temp)] = f_function(f_int(f_temp),f_env(f_temp),f_env(f_temp),f_eff_dot(f_temp),CT(f_temp),RT(f_temp),Af(f_temp),Lce,Fs);
        [f_eff(f_temp),f_eff_dot(f_temp),Tf(f_temp)] = f_function(f_eff(f_temp),f_int(f_temp),f_env(f_temp),f_eff_dot(f_temp),CT(f_temp),RT(f_temp),Af(f_temp),Lce,Fs);
        if f_temp <= index_slow
            Af(f_temp) = Af_slow_function(f_eff(f_temp),Lce,Y_af(f_temp,i));
            OutputAf(f_temp,i)= Af(f_temp);
        else
            Af(f_temp) = Af_fast_function(f_eff(f_temp),Lce,S_af(f_temp,i));
            OutputAf(f_temp,i)= Af(f_temp);
        end
    end
...
output.f_int = Outputfint;
output.f_eff = Outputfeff;
output.Af = OutputAf;
output.S = S_af;
output.Y = Y_af;

...
[~,index_slow] = min(abs(cumsum(Pi)-F_pcsa_slow));
...
for k = 1 %:length(testedUnits)
    ...
    for j = 1:length(FR_test)
        ...
        half_Force = meanForce/2;
        %
        figure(1)
        plot(t,force)
        hold on
        
        [~,index_half_RT] = min(abs(force(1:5*Fs)-half_Force));
        [~,index_half_FT] = min(abs(force(5*Fs:end)-half_Force));
        
        RT_vec(j) = t(index_half_RT)-1;
        FT_vec(j) = t(index_half_FT+5*Fs)-9;
    end   
    figure(2)
    subplot(2,1,1)
    plot(FR_test./FR_half(1),RT_vec)
    subplot(2,1,2)
    plot(FR_test./FR_half(1),FT_vec)
...
T1 = CT; %CT*Lce^2+(CT*1/4)*f_env;
T2_temp = RT; %(RT + (RT*1/4)*Af)/Lce;
T2 = T2_temp/1.68;
t_twitch = 0:1/Fs:1;

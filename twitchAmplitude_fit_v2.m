...
F0 = PCSA*sigma;
N_fibers = 40500; % number of muscle fibers
...
fN_slow = 0.5; % fraction of slow-twitch fibers to the total nubmer of muscle fibers
N_slow = round(fN_slow*N_fibers);
N_fast = N_fibers - N_slow;
PCSA_ratio = 2;
PCSA_slow = PCSA/(1+PCSA_ratio);
PCSA_fast = PCSA_slow*PCSA_ratio;
fPCSA_slow = PCSA_slow/N_slow;
fPCSA_fast = PCSA_fast/N_fast;
...
[error,index_slow] = min(abs(cumsum(ni)-fN_slow*N_fibers));
for m = 1:N_MU
    if m <= index_slow
        Pi(m) = sigma*fPCSA_slow*ni(m);
    else
        Pi(m) = sigma*fPCSA_fast*ni(m);
    end
end
